...
%%%-------------------------------------------------------------------
-module(mf_storage_servant_link_sp).
-author("LENOVO").
...
init(Pars, BH) ->
  base_attributes:write(<<"TaskDurations">>,<<"storage">>,1200000,BH),
  base_attributes:write(<<"TaskDurationList">>,<<"storage">>,[1200000],BH),
  ok.
...
generate_proposal(Requirements, PluginState, NegH, BH) ->
  ...
  {Result,AvailabilityTime} = case StartTime of
                                ...
                                _ ->
                                  TaskDuration = base_attributes:read(<<"TaskDurations">>, <<"storage">>, BH),
