...
%%%-------------------------------------------------------------------
-module(move_storage_master_link_sp).
-author("LENOVO").
...
%%  io:format("Proposal list: ~p~n",[ProposalList]),
  ...
  case map_size(ProposalList) of

    0 ->
      io:format("No proposals~n"),
      %% TODO add case where none meet the requirement
      %% It is when the room that is searched for does not bestaan.
      {ok, [], nostate};
    _ ->
      ...
      io:format("The best time for Move--Storage is: ~p:~p:~p by:~p~n", [Hour+2, Min, Sec, PartnerName]),
...
promise_received(Promise, PluginState, NegH, BH) ->
  FSM_PID = base_variables:read(<<"FSM_INFO">>,<<"FSM_PID">>,BH),
  gen_statem:cast(FSM_PID,storage_available),
  base_variables:write(<<"FSM_INFO">>,<<"FSM_status">>, storage_available,BH),
