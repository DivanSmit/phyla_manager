...
%%%-------------------------------------------------------------------
-module(mf_operator_servant_link_sp).
-author("LENOVO").
...
init(Pars, BH) ->
  base_attributes:write(<<"TaskDurations">>,<<"mf_operator">>,1800000,BH),
  base_attributes:write(<<"TaskDurationList">>,<<"mf_operator">>,[1800000],BH),
  ok.
...
generate_proposal(Requirements, PluginState, NegH, BH) ->
  ...
  {Result,AvailabilityTime} = case StartTime of
                       ...
                       _ ->
                         TaskDuration = base_attributes:read(<<"TaskDurations">>, <<"mf_operator">>, BH),
