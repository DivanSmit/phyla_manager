...
%%%-------------------------------------------------------------------
-module(process_task_sched_tasks_ep).
-author("LENOVO").
...
%%  io:format("Starting with the sched_task task~n"),
  List_of_activities = base_attributes:read(<<"meta">>,<<"activities">>,BH),
  ...
  Data_map = maps:merge(#{<<"parentID">> => MyID,<<"startTime">>=>StartTime}, maps:get(<<"meta">>, Elem)),
...
%%  io:format("Variable updated, newtime: ~p~n",[Times]),
  ...
  if
    ...
    true ->
      ...
      Data_map = maps:merge(#{<<"parentID">> => MyID,<<"startTime">>=>Previous_End_Time}, maps:get(<<"meta">>, Elem)),
