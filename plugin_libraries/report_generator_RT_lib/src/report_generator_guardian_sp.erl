...
%%%-------------------------------------------------------------------
-module(report_generator_guardian_sp).
-author("LENOVO").
...
generate_instance_recipe(Name, ID,BH) ->
  RECIPE = #{
    <<"plugins">>=> [
      #{<<"name">>=><<"report_generator_info_handler_ep">>,<<"lib">>=><<"report_generator_RT_lib">>,<<"init_args">>=>[]}
    ],
    <<"bc">> => #{
      <<"identity">>=>#{
        <<"id">>=>ID,
        <<"name">>=>Name,
        <<"taxonomy">>=>#{<<"arti_class">>=><<"resource-instance">>,<<"base_type">>=><<"REPORT_GENERATOR_TYPE">>}
      },
      <<"capabilities">>=>[<<"REPORT_GENERATOR_INSTANCE_INFO">>],
      <<"responsibilities">>=>[],
      <<"addresses">>=>#{},
      <<"meta">>=>#{}
